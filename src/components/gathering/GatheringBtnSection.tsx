...
import GatheringModal from '../modal/GatheringModal'; // GatheringModal 임포트
...
function GatheringBtnSection() {
  ...
  return (
    <div className="section-wrap mt-[80px] flex flex-col items-center justify-center py-20">
      ...
      <button
        onClick={openModal}
        className="bg-black text-white hover:bg-white hover:text-black shadow-lg duration-400 px-20 py-3 rounded-full text-lg font-semibold"
      >
        나만의 모임 만들기
      </button>

      {/* 모임 생성 모달 */}
      <GatheringModal isOpen={isModalOpen} closeModal={closeModal} />
    </div>
  );
}
