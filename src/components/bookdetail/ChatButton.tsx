...
const ChatButton: React.FC<ChatButtonProps> = ({
  ...
  const handleChatClick = async () => {
    ...
    try {
      // 1. 채팅방 목록을 조회
      const chatroomsResponse = await axios.get('/api/meeting/chatrooms', {
  ...
  const joinChatRoom = async (roomId: number) => {
    try {
      await axios.post('/api/meeting/chat/participates', {
        roomId,
        memberId,
        bookId,
      });
  ...
  const createChatRoom = async () => {
    try {
      const response = await axios.post('/api/meeting/chatrooms', {
        memberId: memberId,
        bookname: bookName,
      });
      const roomId = response.data.roomId;
      navigate(`/ChatPage/${roomId}`); // 새로 생성된 채팅방 페이지로 이동
    } catch (error) {
